...
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM CARDS WHERE ID = @ID_TARJETA)
        BEGIN
        ...
        SELECT
            CL.CLIENT_NAME,
            C.LASTD_CARD,
            C.TOTAL_CREDIT AS SALDO_TOTAL,
            C.CREDIT_LIMIT AS LIMITE_CREDITO,
            C.AV_CREDIT AS SALDO_DISPONIBLE,
            (C.TOTAL_CREDIT * (C.INTEREST / 100)) AS INTERES_BONIFICABLE,
            (C.TOTAL_CREDIT * (C.MIN_INTEREST / 100)) AS CUOTA_MINIMA,
            C.TOTAL_CREDIT AS MONTO_TOTAL_A_PAGAR,
            (C.TOTAL_CREDIT + (C.TOTAL_CREDIT * (C.INTEREST / 100))) AS MONTO_CONTADO_CON_INTERESES,
            @CurrentMonthTotal AS TOTAL_COMPRAS_MES_ACTUAL,
            @PreviousMonthTotal AS TOTAL_COMPRAS_MES_ANTERIOR
        FROM CARDS C
